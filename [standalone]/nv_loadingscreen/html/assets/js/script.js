...
if (showStaffTeam){
	...
	staff_team.forEach(function(user){
		let imgHtml = (user.image && user.image !== "") ? `<img src="${user.image}" class="pfp">` : "";
		$(".staff_team").append(`
			<div class="staff">
				<div class="info">
					${imgHtml}
...
if (theme == "white" || theme == "monochrome" || theme == "blackwhite"){
	$("body").append(`<style>:root{--main:255, 255, 255;}</style>`)
	$("body").css({"background-image":"url('assets/img/red.jpg')", "filter":"grayscale(100%)"})
	$(".winter").css("background","linear-gradient(0deg, rgb(255 255 255 / 10%) 0%, rgba(255, 255, 255, 0.0) 100%)")
}
...
let videoId = getYouTubeId(youtubeVideo);
if (videoId && !enableLocalVideo) {
	if (!showYoutubeVideo){
		videoOpacity = 0
	}
	$("iframe").attr("src", `https://www.youtube.com/embed/${videoId}?autoplay=1&controls=0&enablejsapi=1&disablekb=1&playlist=${videoId}&loop=1`)
